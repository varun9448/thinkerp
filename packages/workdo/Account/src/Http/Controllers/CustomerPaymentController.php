...
class CustomerPaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        if(Auth::user()->can('manage-customer-payments')){
            ...
            return Inertia::render('Account/CustomerPayments/Index', [
                ...
                'filters'       => $request->only(['customer_id', 'status', 'search', 'bank_account_id', 'payment_method']),
                'paymentMethods'=> \Workdo\Account\Models\CustomerPayment::PAYMENT_METHODS,
                'upiSettings'   => [
                    'enabled'  => getSetting('upi_enabled') === 'on',
    ...
    public function store(StoreCustomerPaymentRequest $request)
    {
        if(Auth::user()->can('create-customer-payments')){
            ...
            $payment->payment_method  = $request->payment_method ?? 'other';
