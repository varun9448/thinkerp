...
export default function Index() {
    ...
    const { payments, vendors, bankAccounts, filters: initialFilters, paymentMethods, auth } = usePage<VendorPaymentsIndexProps>().props;
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            sortable: false,
            render: (value: string) => (
                <span className={`px-2 py-0.5 rounded text-xs font-medium ${METHOD_COLORS[value] ?? 'bg-gray-100 text-gray-600'}`}>
                            ...
                            <div className="relative">
                                ...
                                {(() => {
                                    const filtersToCheck = auth.user?.permissions?.includes('manage-users')
                                        ? [filters.vendor_id, filters.status, filters.date_range, filters.bank_account_id, filters.payment_method]
                                        : [filters.status, filters.date_range, filters.bank_account_id, filters.payment_method];
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            hasFilters={!!(filters.search || filters.vendor_id || filters.status || filters.date_range || filters.bank_account_id || filters.payment_method)}
