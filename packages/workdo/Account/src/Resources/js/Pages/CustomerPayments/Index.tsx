...
export default function Index() {
    ...
    const { payments, customers, bankAccounts, filters: initialFilters, paymentMethods, upiSettings, auth } = usePage<CustomerPaymentsIndexProps>().props;
    ...
    const clearFilters = () => {
        setFilters({ search: '', customer_id: '', status: '', payment_method: '', date_range: '' });
        ...
        },
        {
            ...
            render: (value: string) => (
                <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                    value === 'upi'           ? 'bg-green-100 text-green-800' :
                    value === 'neft'          ? 'bg-blue-100 text-blue-800' :
                    value === 'rtgs'          ? 'bg-indigo-100 text-indigo-800' :
                    value === 'imps'          ? 'bg-purple-100 text-purple-800' :
                    value === 'cash'          ? 'bg-amber-100 text-amber-800' :
                    value === 'cheque'        ? 'bg-orange-100 text-orange-800' :
                    value === 'card'          ? 'bg-pink-100 text-pink-800' :
                    value === 'bank_transfer' ? 'bg-cyan-100 text-cyan-800' :
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative">
                                ...
                                />
                                {(() => {
                                    const activeFilters = [filters.customer_id, filters.status, filters.payment_method, filters.date_range].filter(Boolean).length;
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            hasFilters={!!(filters.search || filters.customer_id || filters.status || filters.payment_method || filters.date_range)}
                                                    ...
                                                    <div className="bg-gray-50 rounded-lg p-3">
                                                        ...
                                                        {payment.payment_method && (
                                                            <div className="flex justify-end">
                                                                <span className={`px-2 py-0.5 rounded text-xs font-medium ${METHOD_COLORS[payment.payment_method] ?? 'bg-gray-100 text-gray-600'}`}>
