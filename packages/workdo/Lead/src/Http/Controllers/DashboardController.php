...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-crm-dashboard')) {
            ...
            $totalDealCalls = DealCall::whereHas('deal', fn($q) => $q->where('created_by', creatorId()))->count();
            $totalLeadCalls = LeadCall::whereHas('lead', fn($q) => $q->where('created_by', creatorId()))->count();
