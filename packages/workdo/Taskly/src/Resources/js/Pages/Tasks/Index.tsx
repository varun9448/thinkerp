...
export default function Index() {
    ...
    const tableColumns = [
        ...
        },
        {
            key: 'bonus_stars',
            header: t('Stars'),
            render: (value: number, task: ProjectTask) => {
                const stars = Number(value || 0);
                const leadDays = task.bonus_lead_days;
                return stars > 0 ? `${stars}${leadDays !== undefined && leadDays !== null ? ` / ${leadDays}d` : ''}` : '-';
            }
        },
        {
            key: 'priority',
                                    ...
                                    {tasks.data.map((task) => (
                                        <Card key={task.id} className="border border-gray-200">
                                            <div className="p-4">
                                                ...
                                                <div className="grid grid-cols-2 gap-2 mb-3">
                                                    <div>
                                                        <p className="text-xs font-medium text-gray-600 mb-1">{t('Stars')}</p>
                                                        <p className="text-xs text-gray-900">{Number(task.bonus_stars || 0) > 0 ? task.bonus_stars : '-'}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-xs font-medium text-gray-600 mb-1 text-right">{t('Max Lead')}</p>
                                                        <p className="text-xs text-gray-900 text-right">{task.bonus_lead_days !== undefined && task.bonus_lead_days !== null ? `${task.bonus_lead_days}d` : '-'}</p>
                                                    </div>
                                                </div>
