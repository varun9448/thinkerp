...
export default function Edit({ onSuccess, task, project, milestones, teamMembers, taskStages = [] }: EditProps) {
    ...
    const [data, setData] = useState({
        ...
        priority: 'Medium',
        bonus_stars: 0,
        bonus_lead_days: null as number | null,
        ...
        const fetchTaskData = async () => {
            try {
                ...
                setData({
                    ...
                    bonus_stars: Number(fetchedTask.bonus_stars || 0),
                    bonus_lead_days: fetchedTask.bonus_lead_days ?? null,
        ...
        <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <Label htmlFor="bonus_stars">{t('Stars')}</Label>
                        <Input
                            id="bonus_stars"
                            type="number"
                            min="0"
                            step="0.01"
                            value={data.bonus_stars ?? 0}
                            onChange={(e) => updateData('bonus_stars', parseFloat(e.target.value) || 0)}
                        />
                    </div>
                    <div>
                        <Label htmlFor="bonus_lead_days">{t('Max Lead Days')}</Label>
                        <Input
                            id="bonus_lead_days"
                            type="number"
                            min="0"
                            step="1"
                            value={data.bonus_lead_days ?? ''}
                            onChange={(e) => updateData('bonus_lead_days', e.target.value === '' ? null : parseInt(e.target.value, 10))}
                        />
                    </div>
                </div>
