...
import { Plus, Calendar, Edit, Trash2, MoreVertical, Users, List, Eye, User, Star } from 'lucide-react';
    ...
    const TaskCard = ({ task }: { task: KanbanTask }) => {
        ...
        return (
            ...
            >
                ...
                {Number(task.bonus_stars || 0) > 0 && (
                    <div className="mb-2 inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700">
                        <Star className="h-3 w-3" />
                        <span>{task.bonus_stars}</span>
                        {task.bonus_lead_days !== undefined && task.bonus_lead_days !== null && (
                            <span className="text-amber-600">/ {task.bonus_lead_days}d</span>
                        )}
                    </div>
                )}
