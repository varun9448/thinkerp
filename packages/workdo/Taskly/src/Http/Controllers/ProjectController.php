...
class ProjectController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-project')) {
            $items = Project::query()
                ->select('id', 'name', 'description', 'budget', 'bonus_budget', 'start_date', 'end_date', 'status', 'created_by')
    ...
    public function store(StoreProjectRequest $request)
    {
        if (Auth::user()->can('create-project')) {
            ...
            $project->bonus_budget  = $validated['bonus_budget'] ?? 0;
        ...
        if (Auth::user()->can('view-project') && $project->created_by == creatorId()) {
            ...
            return Inertia::render('Taskly/Project/View', [
                ...
                'projectStats' => [
                    ...
                    'bonusBudget' => $project->bonus_budget,
                ],
                'chartData' => $chartData,
    ...
    public function edit(Project $project)
    {
        if (Auth::user()->can('edit-project') && $project->created_by == creatorId()) {
            return response()->json([
                'project' => $project->only(['id', 'name', 'description', 'budget', 'bonus_budget', 'start_date', 'end_date', 'status'])
            ]);
        } else {
    ...
    public function update(UpdateProjectRequest $request, Project $project)
    {
        if (Auth::user()->can('edit-project')) {
            ...
            $project->description  = $validated['description'];
            $project->budget       = $validated['budget'];
            $project->bonus_budget = array_key_exists('bonus_budget', $validated) ? ($validated['bonus_budget'] ?? 0) : ($project->bonus_budget ?? 0);
            $project->start_date   = $validated['start_date'];
            $project->end_date     = $validated['end_date'];
            $newStatus             = $validated['status'] ?? $project->status;
            $project->status       = $newStatus;

            if ($newStatus === 'Finished' && !$project->finished_at) {
                $project->finished_at = now();
            }

            if ($newStatus !== 'Finished' && !$project->bonus_paid_at) {
    ...
    public function duplicate(Request $request, Project $project)
    {
        if (Auth::user()->can('duplicate-project')) {
            ...
            $newProject->finished_at = $newProject->status === 'Finished' ? now() : null;
            $newProject->bonus_paid_at = null;
            $newProject->bonus_payroll_id = null;
            ...
            if ($request->get('all') || $request->get('tasks')) {
                foreach ($originalProject->tasks ?? [] as $task) {
                    ...
                    $newTask->bonus_awarded_at = null;
                    $newTask->bonus_awarded_to = null;
                    $newTask->save();
