...
class ProjectTaskController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-project-task')) {
            ...
            $query = ProjectTask::select('project_tasks.id', 'project_id', 'milestone_id', 'title', 'priority', 'bonus_stars', 'bonus_lead_days', 'assigned_to', 'duration', 'description', 'stage_id', 'completed_at', 'bonus_awarded_at', 'created_at')
    ...
    public function store(StoreProjectTaskRequest $request)
    {
        if (Auth::user()->can('create-project-task')) {
            ...
            $task->bonus_stars = array_key_exists('bonus_stars', $validated) ? ($validated['bonus_stars'] ?? 0) : ($task->bonus_stars ?? 0);
            $task->bonus_lead_days = array_key_exists('bonus_lead_days', $validated) ? $validated['bonus_lead_days'] : $task->bonus_lead_days;
            ...
            $task->syncBonusCompletion(TaskStage::find($task->stage_id));
            $task->save();
            ...
            foreach ($stages as $stage) {
                ...
                $stageTasks = $allTasks->where('stage_id', $stage->id)->map(function ($task) {
                    ...
                    return [
                        ...
                        'bonus_stars' => $task->bonus_stars,
                        'bonus_lead_days' => $task->bonus_lead_days,
                        'completed_at' => $task->completed_at,
                        'bonus_awarded_at' => $task->bonus_awarded_at,
    ...
    public function move(Request $request, ProjectTask $task)
    {
        ...
        if ($canMove) {
            ...
            $task->syncBonusCompletion($newStage);
            $task->save();
    ...
    public function getSubmissions(ProjectTask $task)
    {
        if (!Auth::user()->can('manage-project-task')) {
            return response()->json(['error' => __('Permission denied')], 403);
        }

        $submissions = TaskWorkSubmission::where('task_id', $task->id)
            ->with(['user:id,name,avatar', 'approvedBy:id,name'])
    ...
    public function submitWork(Request $request, ProjectTask $task)
    {
        $assignedUserIds = array_map('strval', $task->assignedUserIds());

        if (!in_array((string) Auth::id(), $assignedUserIds, true)) {
            return response()->json(['error' => __('You are not assigned to this task.')], 403);
        }

        $existing = TaskWorkSubmission::where('task_id', $task->id)
            ->where('user_id', Auth::id())
            ->first();

        if ($existing) {
            return response()->json(['error' => __('You have already submitted your work for this task.')], 422);
        }

        $request->validate(['notes' => 'nullable|string|max:1000']);

        TaskWorkSubmission::create([
            'task_id'      => $task->id,
            'user_id'      => Auth::id(),
            'notes'        => $request->notes,
            'submitted_at' => now(),
        ...
        return response()->json(['success' => true, 'message' => __('Your work has been submitted for review.')]);
    }

    public function approveWork(TaskWorkSubmission $submission)
    {
        return $this->resolveSubmission($submission, [
            'status'           => 'approved',
            'approved_by'      => Auth::id(),
            'approved_at'      => now(),
    ...
    public function rejectWork(Request $request, TaskWorkSubmission $submission)
    {
    ...
    private function resolveSubmission(TaskWorkSubmission $submission, array $data, string $message)
    {
        if (!Auth::user()->can('manage-project-task')) {
            return response()->json(['error' => __('Permission denied')], 403);
        }

        $task = $submission->task()->with(['taskStage', 'project'])->first();

        if (!Auth::user()->can('manage-any-project-task') && $task->creator_id !== Auth::id()) {
            return response()->json(['error' => __('Permission denied')], 403);
        }

        $submission->update($data);
        $task->syncBonusCompletion();
        $task->save();
