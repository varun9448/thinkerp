    ...
    public function index(Request $request)
    {
        try {
            if (Auth::user()->can('manage-project-task')) {
                $items = ProjectTask::select('project_tasks.id', 'project_id', 'milestone_id', 'title', 'priority', 'bonus_stars', 'bonus_lead_days', 'assigned_to', 'duration', 'description', 'stage_id', 'completed_at', 'bonus_awarded_at', 'created_at')
    ...
    private function createTask(Request $request)
    {
        if (Auth::user()->can('create-project-task')) {
            $validator = Validator::make($request->all(), [
                ...
                'bonus_stars'  => 'nullable|numeric|min:0',
                'bonus_lead_days' => 'nullable|integer|min:0',
