...
import { filterFrontendPackages } from './frontend-packages';
...
const getPackageSettingsItems = (userRoles: string[], activatedPackages: string[], t: (key: string) => string): SettingMenuItem[] => {
    ...
    const allModules = userRoles.includes('superadmin')
        ? import.meta.glob([
            ...
            '!../../../packages/workdo/Pos/src/Resources/js/settings/superadmin-setting.ts',
        ], { eager: true })
        : import.meta.glob([
            '../../../packages/workdo/*/src/Resources/js/settings/company-setting.ts',
            '!../../../packages/workdo/Pos/src/Resources/js/settings/company-setting.ts',
        ], { eager: true });

    filterFrontendPackages(Array.isArray(activatedPackages) ? activatedPackages : []).forEach(packageName => {
