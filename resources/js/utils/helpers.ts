...
import { filterFrontendPackages, isPackageFrontendDisabled } from './frontend-packages';
...
const isPackageActive = (packageName: string, pageProps?: any): boolean => {
  try {
    if (isPackageFrontendDisabled(packageName)) {
      return false;
    }
    ...
    return filterFrontendPackages(activatedPackages).includes(packageName);
  } catch {
    return false;
  }
};
...
const adminPackages = (pageProps?: any): string[] => {
    try {
      ...
      return filterFrontendPackages(packages.filter((pkg: any) => pkg.is_enable === true).map((pkg: any) => pkg.name));
    } catch {
      return [];
    }
  };
...
const parseAssignedIds = (raw: any): string[] => {
    if (!raw) return [];
